...
function utf8ToBytes(str) {
  // encodeURIComponent produces UTF-8 escaped bytes; decode to byte array
...
function sha256Bytes(bytes) {
  ...
  // Append 64-bit big-endian length, split into high/low 32-bit words
  // (JS `>>>` wraps shift amount modulo 32, so we must split manually)
...
function hmacSha256Hex(keyStr, messageStr) {
  ...
  const innerInput = iKeyPad.concat(messageBytes)
  const innerHash = sha256Bytes(innerInput)
  const outerInput = oKeyPad.concat(innerHash)
...
function computeSignature(method, uri, timestamp, nonce) {
  const stringToSign = [method, uri, timestamp, nonce].join('\n')
  return hmacSha256Hex(SIGNING_SECRET, stringToSign)
}

export function signRequest(method, url) {
  const timestamp = Date.now().toString()
  const nonce = generateNonce()
  // BASE_URL already includes /api suffix, but the backend sees /api/xxx in requestURI
  // The url parameter here is relative like "/user/info"
  const uri = '/api' + url
  const signature = computeSignature(method.toUpperCase(), uri, timestamp, nonce)
