    ...
    <div class="container">
      <div class="card">
        ...
        <div class="category-grid">
          <div class="category-item" v-for="cat in visibleCategories" :key="cat.id" @click="$router.push(`/category/${cat.id}`)">
            <el-icon :size="32"><Folder /></el-icon>
            <span class="cat-name">{{ cat.name }}</span>
          </div>
        </div>
        <div class="load-more" v-if="categories.length > visibleCount">
          <el-button text type="primary" @click="showMore">
            加载更多 ({{ visibleCount }}/{{ categories.length }})
          </el-button>
        </div>
...
const visibleCategories = computed(() => categories.value.slice(0, visibleCount.value))
